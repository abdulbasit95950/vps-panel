...
def get_active_users_and_pids():
    user_pids = {}
    try:
        ps_out = subprocess.check_output(["ps", "aux"], stderr=subprocess.DEVNULL).decode("utf-8", errors="ignore")
        for line in ps_out.splitlines():
            if "sshd:" not in line or "grep" in line:
                continue
            m = re.search(r"sshd:\s+([a-zA-Z0-9_.-]+)(@|\s+\[)", line)
            if not m:
                continue
            uname = m.group(1)
            parts = line.split()
...
while True:
    try:
        user_pids_map = get_active_users_and_pids()
...
def add_user(username, password, days, ip_limit, gb_limit):
    ...
    r = sh(["useradd", "-M", "-s", "/usr/sbin/nologin", "-e", exp_date, username])
    if r.returncode != 0:
...
def connected_ips_text():
    r = sh(["ss", "-tnp"])
    lines = [l for l in r.stdout.splitlines() if (":109" in l or ":444" in l or ":447" in l) and "ESTAB" in l]
...
def fix_websocket():
    ...
    sh(["systemctl", "restart", "stunnel4"])
    sh(["systemctl", "daemon-reload"])
    sh(["systemctl", "restart", "ws-proxy"])
    sh(["systemctl", "restart", "autokill"])
    apply_nginx_config()
    return "OpenSSH, Stunnel, WebSocket & Bandwidth engine restarted."
...
def configure_openssh():
    ...
    backup = "/etc/ssh/sshd_config.rr-bak"
    if not os.path.exists(backup):
        sh(["cp", sshd_conf, backup])
    for line in ["Port 22", "Port 109", "Port 447", "Banner /etc/issue.net", "PasswordAuthentication yes"]:
        ensure_line(sshd_conf, line)
    test = sh(["sshd", "-t"])
    if test.returncode == 0:
        sh(["systemctl", "restart", "ssh"])
        return True
    sh(["cp", backup, sshd_conf])
    return False
...
def setup_stunnel():
    ...
    if dom != "No Domain Set" and os.path.exists(f"/etc/letsencrypt/live/{dom}/fullchain.pem"):
        cert_pem = f"/etc/letsencrypt/live/{dom}/fullchain.pem"
        key_pem = f"/etc/letsencrypt/live/{dom}/privkey.pem"
    else:
        os.makedirs("/etc/stunnel/selfcert", exist_ok=True)
        if not os.path.exists("/etc/stunnel/selfcert/stunnel.pem"):
            run(
                "openssl req -x509 -nodes -days 3650 -newkey rsa:2048 "
                "-keyout /etc/stunnel/selfcert/stunnel.pem "
                "-out /etc/stunnel/selfcert/stunnel.pem "
                "-subj '/CN=raretriccks'"
            )
        cert_pem = "/etc/stunnel/selfcert/stunnel.pem"
        key_pem = "/etc/stunnel/selfcert/stunnel.pem"
    ...
    ensure_line("/etc/default/stunnel4", "ENABLED=1")

    sh(["systemctl", "daemon-reload"])
    sh(["systemctl", "enable", "stunnel4"])
    sh(["systemctl", "restart", "stunnel4"])


def open_firewall_ports():
    sh(["bash", "-c", "command -v ufw >/dev/null || apt-get install -y ufw"])
    for p in ["22", "80", "109", "443", "444", "447"]:
        sh(["ufw", "allow", f"{p}/tcp"])
        run(f"iptables -C INPUT -p tcp --dport {p} -j ACCEPT 2>/dev/null || iptables -I INPUT -p tcp --dport {p} -j ACCEPT")
    sh(["bash", "-c", "ufw --force enable"])
...
def install_components_sync():
    ...
    sh([
        ...
        "socat", "jq", "openssl", "nginx", "openssh-server", "stunnel4", "ufw",
        "certbot", "python3", "python3-pip", "lsof", "iptables",
    ])

    # 2) remove legacy dropbear (migrate to OpenSSH)
    sh(["systemctl", "stop", "dropbear"])
    sh(["systemctl", "disable", "dropbear"])
    sh(["apt-get", "purge", "-y", "dropbear", "dropbear-run", "dropbear-bin"])
    ...
    # 4) OpenSSH (ports 22, 109, 447) + Stunnel (SSL port 444)
    configure_openssh()
    setup_stunnel()
    ...
    # 7) nginx (only if domain already set) + firewall
    apply_nginx_config()
    open_firewall_ports()
...
def uninstall_all():
    ...
    sh(["systemctl", "stop", "stunnel4"])
    sh(["systemctl", "disable", "stunnel4"])
    try:
        os.remove("/etc/stunnel/stunnel.conf")
    except FileNotFoundError:
        pass
    if os.path.exists("/etc/ssh/sshd_config.rr-bak"):
        sh(["cp", "/etc/ssh/sshd_config.rr-bak", "/etc/ssh/sshd_config"])
        sh(["systemctl", "restart", "ssh"])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        await q.edit_message_text("\U0001F4E6 Poora system install ho raha hai (OpenSSH + Stunnel + WebSocket + Auto-Kill + Firewall), 2-5 min lagega...")
        await asyncio.to_thread(install_components_sync)
        await q.message.reply_text(
            "\u2705 Installation complete! Dropbear hata di gayi, OpenSSH (22/109/447), Stunnel SSL (444), "
            "Banner, WebSocket Proxy, Auto-Kill/Bandwidth service aur firewall ports (22,80,109,443,444,447) "
            "sab deploy/open ho gaye hain.\n"
...
configure_openssh() {
    ...
    local backup="/etc/ssh/sshd_config.rr-bak"
    [[ -f "$backup" ]] || cp "$sshd_conf" "$backup"

    for line in "Port 22" "Port 109" "Port 447" "Banner /etc/issue.net" "PasswordAuthentication yes"; do
        grep -qxF "$line" "$sshd_conf" || echo "$line" >> "$sshd_conf"
    done
    ...
    if sshd -t; then
        ...
        echo -e "${RED}[ERROR] sshd_config me syntax error aa gaya! Restart skip kar diya, purani config restore ki ja rahi hai.${NC}"
        cp "$backup" "$sshd_conf"
        return 1
    fi
}
...
setup_stunnel() {
    ...
    if [[ "$dom" != "No Domain Set" && -f "/etc/letsencrypt/live/$dom/fullchain.pem" ]]; then
        cert_pem="/etc/letsencrypt/live/$dom/fullchain.pem"
        key_pem="/etc/letsencrypt/live/$dom/privkey.pem"
    else
        mkdir -p /etc/stunnel/selfcert
        if [[ ! -f /etc/stunnel/selfcert/stunnel.pem ]]; then
            openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
                -keyout /etc/stunnel/selfcert/stunnel.pem \
                -out /etc/stunnel/selfcert/stunnel.pem \
                -subj "/CN=raretriccks" 2>/dev/null
        fi
        cert_pem="/etc/stunnel/selfcert/stunnel.pem"
        key_pem="/etc/stunnel/selfcert/stunnel.pem"
    fi

    cat << STUN_EOF > /etc/stunnel/stunnel.conf
pid = /var/run/stunnel4/stunnel.pid
cert = ${cert_pem}
key = ${key_pem}
...
open_firewall_ports() {
    echo -e "${BLUE}Opening required ports in firewall (22, 80, 109, 443, 444, 447)...${NC}"
    if ! command -v ufw &>/dev/null; then
        apt install -y ufw
    fi
    for p in 22 80 109 443 444 447; do
...
install_all_components() {
    ...
    apt install -y curl wget unzip tar net-tools socat jq openssl nginx openssh-server stunnel4 ufw certbot python3 python3-pip lsof iptables

    echo -e "${BLUE}[3/7] Removing legacy Dropbear (migrating to OpenSSH)...${NC}"
    systemctl stop dropbear 2>/dev/null
    systemctl disable dropbear 2>/dev/null
    apt purge -y dropbear dropbear-run dropbear-bin 2>/dev/null

    echo -e "${BLUE}[4/7] Configuring OpenSSH (Ports 22, 109, 447) & Banner...${NC}"
...
BANNER_EOF
    ...
    echo -e "${BLUE}[5/7] Configuring Stunnel (SSH over SSL, Port 444)...${NC}"
    setup_stunnel

    echo -e "${BLUE}[6/7] Creating Multi-Payload Python WebSocket Service...${NC}"
...
SVC_EOF
    ...
    echo -e "${BLUE}[7/7] Installing Bandwidth Tracking Engine & Opening Firewall Ports...${NC}"
    install_python_tracker
    open_firewall_ports

    echo -e "\n${GREEN}[SUCCESS] Base components Installed (OpenSSH + Stunnel + WebSocket + Auto-Kill)!${NC}"
    press_any_key
}
...
check_connected_ips() {
    ...
    mapfile -t external_ips < <(ss -tnp 2>/dev/null | grep -E ":(80|109|443|444|447)" | grep "ESTAB" | awk '{print $5}' | cut -d: -f1 | grep -vE "^127\.|^::1" | sort -u)
    ...
    while IFS= read -r line; do
        local username=$(echo "$line" | grep -oP 'sshd:\s+\K[a-zA-Z0-9_.-]+(?=(@|\s+\[))')
        [[ -z "$username" ]] && continue
        local final_ip="${real_ip_pool[$ip_index]:-Direct/Unknown}"
        ip_index=$((ip_index + 1))
        printf " User: %-18s | IP/Source: %-25s [ONLINE]\n" "$username" "$final_ip"
        total_count=$((total_count + 1))
    done < <(ps aux | grep 'sshd:' | grep -v grep)
    ...
    while true; do
        ...
        case $u_choice in
            1)
                ...
                useradd -M -s /usr/sbin/nologin -e "$exp_date" "$username"
                if [[ $? -ne 0 ]]; then
                ...
                echo -e " SSH SSL      : ${CYAN}444 (Stunnel)${NC}"
...
status_check() {
    ...
    local stunnel_badge="${RED}[ INACTIVE ]${NC}"
    ...
    [[ "$stunnel_status" == "active" ]] && stunnel_badge="${GREEN}[ ACTIVE ]${NC}"
    ...
    printf "   %-28s : %b\n" "Stunnel SSL Engine" "$stunnel_badge"
...
uninstall_panel() {
    ...
    echo -e "  - Stunnel SSL config aur sshd_config changes revert"
    ...
    if [[ -f /etc/ssh/sshd_config.rr-bak ]]; then
        cp /etc/ssh/sshd_config.rr-bak /etc/ssh/sshd_config
    ...
    echo -e "${YELLOW}[NOTE] Nginx, OpenSSH, Stunnel, Certbot packages khud remove nahi kiye gaye.${NC}"
    echo -e "${YELLOW}       Poori tarah hataane ke liye manually chalayein: apt remove --purge nginx stunnel4 certbot${NC}"
